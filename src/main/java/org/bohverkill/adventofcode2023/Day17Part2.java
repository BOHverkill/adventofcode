...
//        String input = Utils.getInput("/2023/Day17_2_1_example");
...
//            System.out.println(current + " " + gScore.get(current));
            ...
            if (current.column() == goal.column() && current.row() == goal.row() && (current.countSameDirection() >= 3 && current.countSameDirection() < 10)) {
    ...
    public static final class Node implements Comparable<Node> {
        ...
        private static void buildAndAdd(int row, int column, Direction direction, int countSameDirection, int[][] heatLossMap, Set<Node> neighbors) {
            if (!isNotValid(row, column, heatLossMap.length, heatLossMap[0].length) && countSameDirection < 10) {
